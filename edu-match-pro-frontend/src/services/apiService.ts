...
class ApiService {
   ...
   */
  async chatWithAgent(query: string, sessionId?: string): Promise<{ response: string }> {
    return this.request<{ response: string }>('/ai/agent', {
      method: 'POST',
      body: JSON.stringify({
        query,
  ...
  /**
   * @deprecated 使用 chatWithAgent 代替
   */
  async extractAIParameters(query: string, conversationHistory: any[] = []): Promise<any> {
    console.warn('extractAIParameters is deprecated. Please use chatWithAgent.');
    return { extracted_params: {}, followup_question: null, is_complete: true };
  }

  /**
   * @deprecated 使用 chatWithAgent 代替
   */
  async analyzeAIStrategy(userParams: any, conversationHistory: any[] = []): Promise<any> {
    console.warn('analyzeAIStrategy is deprecated. Please use chatWithAgent.');
    return { report: 'Deprecated', school_data: {}, statistics: {} };
  }
}
